...
| **wicked-brain** | The memory layer — cross-session recall, cited search, `smaht:briefing`. Carries decisions/gotchas from session 1 to session 47. | structural search (`blast-radius`/`lineage`/`hotspots`, via wicked-estate) still works; you lose cross-session memory + semantic search. | `/plugin install wicked-brain` (npm `0.14.0`) |
