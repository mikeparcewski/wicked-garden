...
|-------|-----------------|--------------|---------|
| **wicked-estate** | The memory/knowledge layer — cross-session memory, cited knowledge recall, and the code graph. Carries decisions/gotchas from session 1 to session 47. | hooks fail open; you lose cross-session memory + knowledge recall (and graph queries degrade to Grep). | Install the `wicked-estate` + `wicked-estate-mcp` binaries onto PATH or `~/.local/bin` |
