# smaht briefing — What Happened Since Last Session Rubric

Full rubric for the `briefing` sub-action of `skills/smaht/SKILL.md`
(formerly `commands/smaht/briefing.md`, retired in the skills-only consolidation).
Generate a "what happened since last session" briefing. Primary source: unified
event log. Fallback: individual domain queries. NOT for live session-state
inspection — that is the `state` sub-action (`refs/state.md`).

Args: `[--days N] [--project name]`
