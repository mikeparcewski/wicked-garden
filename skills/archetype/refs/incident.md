...
1. Look at dashboards FIRST — the `wicked-garden-platform-health` module and the
   `wicked-garden-platform` skill's `traces` / `incident` actions. Hypothesize from data, not memory.
...
the `wicked-garden-core` skill's `setup` action.
