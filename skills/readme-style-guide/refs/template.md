...
/wicked-garden:{domain}-{cmd1} {args}
/wicked-garden:{domain}-{cmd2} {args}
```
...
|---------|-------------|---------|
| `/wicked-garden:{domain}-{cmd}` | {Active-voice description} | `/wicked-garden:{domain}-{cmd} {args}` |
...
| Command | Use When |
|---------|----------|
| `/wicked-garden:{domain}-{cmd1}` | {Trigger condition — when this is the right choice} |
...
<!-- CONDITIONAL: Required if plugin exposes structured data via DomainStore. -->
## Data API

Exposes data via DomainStore:
