...
**Hand-off** — open this skill, `wicked-garden-platform-prereq-doctor`, with `check gh` as the argument; on Claude Code this is the Skill tool (or let the PostToolUseFailure hook catch the missing tool automatically — just try to use it), on any other seat open the named skill from your catalog and run it inline, then continue here.
