...
python3 scripts/glab_ops.py mr-status 123 --project group/name
```
...
# With notes
python3 scripts/glab_ops.py release --notes "Breaking change"
```
...
```bash
# Pretty print
python3 scripts/glab_ops.py diagnose | jq '.'

# Extract errors
python3 scripts/glab_ops.py diagnose | jq '.errors[].message'
```
