...
1. **Factor analysis** — the `wicked-garden:propose-process` facilitator rubric identifies change signals via its 9-factor reading (reversibility, blast_radius, state_complexity, etc.) rather than keyword-based signals. See the facilitator's `process-plan.md` for the factor table.
