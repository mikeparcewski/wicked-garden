...
### PRE-2: Clean state
- **Check**: `rm -rf {SM_LOCAL_ROOT}/wicked-mem/memories/ 2>/dev/null; echo "clean"`
...
- **Check**: Run `rm -rf {SM_LOCAL_ROOT}/wicked-mem/memories/ 2>/dev/null; echo "clean"`
...
- **Evidence required**:
  ...
  - `step-1-state` — Run `ls {SM_LOCAL_ROOT}/wicked-mem/memories/*.json 2>/dev/null | wc -l`
- **Assertions**:
  - `step-1-output` CONTAINS "stored" OR CONTAINS "saved" OR CONTAINS "created"
  - `step-1-output` NOT_CONTAINS "error"
