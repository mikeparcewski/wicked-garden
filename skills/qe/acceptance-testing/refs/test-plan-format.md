...
### PRE-2: Clean state
- **Check**: `rm -rf $(python3 "${CLAUDE_PLUGIN_ROOT}/scripts/resolve_path.py" <domain>)/wicked-mem/memories/ 2>/dev/null; echo "clean"`
...
- **Check**: Run `rm -rf $(python3 "${CLAUDE_PLUGIN_ROOT}/scripts/resolve_path.py" <domain>)/wicked-mem/memories/ 2>/dev/null; echo "clean"`
...
- **Evidence required**:
  ...
  - `step-1-state` — Run `ls $(python3 "${CLAUDE_PLUGIN_ROOT}/scripts/resolve_path.py" <domain>)/wicked-mem/memories/*.json 2>/dev/null | wc -l`
- **Assertions**:
  - `step-1-output` CONTAINS "stored" OR CONTAINS "saved" OR CONTAINS "created"
  - `step-1-output` NOT_CONTAINS "error"
