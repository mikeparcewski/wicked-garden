...
```bash
/wicked-mem:store "Chose PostgreSQL over MongoDB for payment system. \
...
```bash
/wicked-mem:store "Authentication flow: 1) Validate JWT on backend, \
...
```bash
/wicked-mem:store "Session bug 2026-01-18: JWT migration broke session \
