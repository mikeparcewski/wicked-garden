---
description: Setup instructions and usage for wicked-kanban
---

# /wicked-kanban:help

AI-native kanban that serves as the **source of truth** for task management.

## How It Works

wicked-kanban automatically captures all task activity:

- **TaskCreate/TaskUpdate** → Synced to kanban via hooks
- **Git commits** → Linked to active task
- **Agent work** → Logged as comments
- **Repo binding** → Each repo maps to a project

## Available Commands

| Command | Purpose |
|---------|---------|
| `/wicked-kanban:help` | This help |
| `/wicked-kanban:board-status` | Show current board state |
| `/wicked-kanban:new-task` | Quick task creation |
| `/wicked-kanban:name-session` | Name the current session |
| `/wicked-kanban:start-api` | Start data API for workbench |

## CLI Usage

```bash
cd ${CLAUDE_PLUGIN_ROOT}

# List projects
uv run python scripts/kanban.py list-projects

# List tasks
uv run python scripts/kanban.py list-tasks PROJECT_ID --swimlane todo

# Create task
uv run python scripts/kanban.py create-task PROJECT_ID "Task name" --priority P1

# Update task
uv run python scripts/kanban.py update-task PROJECT_ID TASK_ID --swimlane in_progress

# Add comment
uv run python scripts/kanban.py add-comment PROJECT_ID TASK_ID "Comment text"

# Link commit
uv run python scripts/kanban.py add-commit PROJECT_ID TASK_ID abc1234

# View activity
uv run python scripts/kanban.py activity PROJECT_ID --limit 20

# Search
uv run python scripts/kanban.py search "query"
```

## Data Structure

```
~/.something-wicked/wicked-kanban/
├── config.json              # Repo → project mappings
├── active_context.json      # Current task/session
└── projects/
    └── {id}/
        ├── project.json     # Metadata
        ├── swimlanes.json   # Board columns
        ├── tasks/           # One file per task
        ├── initiatives/     # Sessions/sprints
        └── activity/        # Daily logs (JSONL)
```

## Configuration

| Setting | Default | Environment Variable |
|---------|---------|---------------------|
| Data Dir | ~/.something-wicked/wicked-kanban | `WICKED_KANBAN_DATA_DIR` |

## Rendering

The board is rendered by **wicked-workbench** dashboard components.

To enable workbench rendering:
1. Start the data API: `/wicked-kanban:start-api`
2. Use wicked-workbench to generate A2UI dashboards
3. The API provides data at `http://localhost:18888`

### Available Components

See `.claude-plugin/catalog.json` for full component definitions:
- `KanbanBoard`, `Swimlane`, `TaskCard` - Board views
- `TaskList`, `TaskDetailPanel` - Task displays
- `InitiativeCard`, `InitiativeList` - Initiative views
- `DependencyGraph`, `TraceabilityLinks` - Relationships
- `CommentsPanel`, `ArtifactsPanel`, `CommitsList` - Details
