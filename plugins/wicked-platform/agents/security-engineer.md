...
```
/wicked-search:code "password|secret|api_key|token" --path {target}
/wicked-search:code "eval\(|exec\(|system\(" --path {target}
```
...
- Use `/wicked-platform:github-actions` for secure pipeline design
- Use `/wicked-platform:gh-cli` for security advisory management
- Use `/wicked-platform:gitlab-ci` for GitLab security features
