...
Find version files:
```
/wicked-search:code "version|package\.json|pyproject\.toml|Cargo\.toml|pom\.xml" --path {target}
```
...
- Use `/wicked-platform:gh-cli` for GitHub release automation
- Use `/wicked-platform:github-actions` for release pipelines
- Use `/wicked-platform:glab-cli` for GitLab release automation
