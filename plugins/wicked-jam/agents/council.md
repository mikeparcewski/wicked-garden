...
Consider running /wicked-jam:brainstorm first to generate candidates,
...
|------------------------|----------|
| 0 | Refuse council. Suggest `/wicked-jam:brainstorm` instead. |
...
Suggestion: Use /wicked-jam:brainstorm for single-model exploration,
...
```
/wicked-mem:store "Council: {topic} → {verdict_summary}" --type decision --tags "council,{topic_slug}" --importance high
```
