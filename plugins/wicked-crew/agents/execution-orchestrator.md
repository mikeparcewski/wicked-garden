...
- **wicked-engineering:review**: Full multi-perspective code review
...
```
/wicked-mem:recall "QE Strategy Gate {target}"
```
...
```
/wicked-engineering:review {target}
```
...
```
Task(subagent_type="wicked-qe:risk-assessor",
...
```
/wicked-mem:store "Execution Gate: {decision} for {target}. {rationale}" --type decision --tags qe,gate,execution
```
