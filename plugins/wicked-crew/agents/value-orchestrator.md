...
- **wicked-product:requirements-analyst**: Deep requirements analysis
...
```
/wicked-product:strategy {target}
```
...
```
/wicked-mem:store "Value Gate: {decision} for {target}. {rationale}" --type decision --tags qe,gate,value
```
