...
If phase is `pending` or `in_progress`, inform user they need to run `/wicked-crew:execute` first.
...
- Report: "Cannot approve: {count} stale tasks detected. Run /wicked-crew:execute to recover."
...
if not signoff:
    BLOCK_APPROVAL("No sign-off recorded. Run /wicked-crew:execute to get phase reviewed.")
...
Phase progression follows the `phase_plan` stored in project.json (set during `/wicked-crew:start`):
