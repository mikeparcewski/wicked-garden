...
Recall preferences:
```
/wicked-garden:mem-recall "stakeholder {name} reporting preferences"
```
...
```
/wicked-garden:search-code "outcome|summary|decision" --path phases/
```
...
Use reporting skill:
```
/wicked-garden:delivery-report {data_export} --personas delivery-lead,product-lead
```
...
```
/wicked-garden:mem-store "stakeholder {name} prefers {format} reports on {frequency} covering {topics}"
```
