...
Creates MAPS_TO relationships between ActiveRecord models and database columns.
"""
...
@register_linker
class RubyORMLinker(BaseLinker):
    """Links ActiveRecord models to database columns."""

    name = "ruby_orm"
    description = "Links ActiveRecord models to database columns"
    priority = 20
    ...
    def link_all(self) -> int:
        """Link all Ruby ORM model fields to their columns."""
        links_created = 0

        entity_fields = self.graph.find_by_type(SymbolType.ENTITY_FIELD)

        for field in entity_fields:
            if not field.file_path or not field.file_path.endswith('.rb'):
                continue

            metadata = field.metadata or {}
            if metadata.get('orm_type') != 'activerecord':
                continue
            ...
            try:
                ...
                logger.warning(f"Failed to link Ruby field {field.id}: {e}")

        logger.info(
            f"Created {links_created} Ruby ORM links, "
            f"{len(self._created_columns)} column symbols"
        )
        return links_created

    def _link_field_to_column(self, field: Symbol) -> List[Reference]:
        """Link a Ruby ORM field to its database column."""
        references = []
        metadata = field.metadata or {}

        column_name = metadata.get("column_name")
        if not column_name:
            # Skip relationship fields without column
            return references
