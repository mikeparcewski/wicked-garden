; Commonlisp tree-sitter queries for extracting code symbols and relationships
; Converted from commonlisp-tags.scm

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Function Definitions ;;;;;;;;;;;;;;;;;;;;;;;

(defun_header
  function_name: (sym_lit) @code_function.name) @code_function.def
...
;;; But we must exclude some cases. Note, tree-sitter @_ignore
...
;;
(defun_header
  lambda_list: (list_lit . [(sym_lit) (package_lit)] @_ignore))
...
(defun_header
  lambda_list: (list_lit (list_lit . [(sym_lit) (package_lit)] @_ignore)))
...
(list_lit . [(sym_lit) (package_lit)] @call.function
          . (list_lit (list_lit . [(sym_lit) (package_lit)] @_ignore))
          (#match? @call.function
...
;;         within the scope, the usual  @call.ref within
...
;; first element
(list_lit . [(sym_lit) (package_lit)] @call.function) @call.ref

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; classes

(list_lit . [(sym_lit) (package_lit)] @_ignore
          . [(sym_lit) (package_lit)] @code_class.name
  (#match? @_ignore "(?i)^(cl:)?defclass$")
          ) @code_class.def

(list_lit . [(sym_lit) (package_lit)] @_ignore
          . (quoting_lit [(sym_lit) (package_lit)] @code_class.ref)
  (#match? @_ignore "(?i)^(cl:)?make-instance$")
          ) @code_class.ref

;;; TODO:
;;  - @code_class.ref for base classes
...
;;     allows any @.ref* and @.def* tags)
;;     Probably it's better to just use @call.ref for all
