...
/wicked-search:docs "authentication requirements"

# 3. Narrow to code if looking for implementation
/wicked-search:code AuthService
```
...
/wicked-search:code UserService
...
/wicked-search:docs "REQ-AUTH-001"

# 2. Find implementing code
/wicked-search:impl "REQ-AUTH-001"

# 3. Verify coverage
/wicked-search:refs <each implementing symbol>
```
...
- `/wicked-search:docs "rate limiting"` (exact phrase)
- `/wicked-search:docs rate limiting` (both words separately)

### Combine Searches
```
# Search docs for feature name
/wicked-search:docs "OAuth integration"

# Then search code for mentioned symbols
/wicked-search:code OAuthClient
```

## When Search Doesn't Find Results

1. **Check if indexed**: `/wicked-search:stats`
2. **Re-index**: `/wicked-search:index . --force`
