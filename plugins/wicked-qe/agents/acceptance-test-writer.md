...
- **Scenarios**: Use wicked-scenarios:check to validate scenario format
...
```markdown
### 1. Store a memory
/wicked-mem:store "Use JWT tokens" --type decision
...
- Action: Run /wicked-mem:store "Use JWT tokens" --type decision
- Assert: It works
```
...
- Action: Invoke Skill tool with skill="wicked-mem:store", args="\"Use JWT tokens\" --type decision --tags auth"
- Evidence required:
