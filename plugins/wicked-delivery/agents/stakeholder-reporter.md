...
- **Delivery metrics**: Use /wicked-delivery:report for computed metrics
- **Kanban**: Use wicked-kanban for current task data
- **Memory**: Use wicked-mem for historical comparison
- **Risk**: Use wicked-delivery:risk-monitor for risk context
...
```
/wicked-mem:recall "sprint report {previous_sprint}"
```
...
```
/wicked-mem:store "Sprint {name} report: {completion_rate}% complete, {blocker_count} blockers, status {status}" --type discovery
```
