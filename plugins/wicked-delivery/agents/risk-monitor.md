...
Find warning signs:
```
/wicked-search:code "TODO|FIXME|HACK|blocked|delayed" --path .
```
...
Recall past risks:
```
/wicked-mem:recall "risk {project_type}"
```
...
Store risk patterns:
```
/wicked-mem:store "risk pattern: {risk type} in {context} led to {impact} - mitigated by {action}"
```

Recall for new projects:
```
/wicked-mem:recall "risk {similar project type}"
```
