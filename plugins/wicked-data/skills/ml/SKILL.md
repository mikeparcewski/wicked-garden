...
```bash
/wicked-data:ml review path/to/model/
```
...
```bash
/wicked-data:ml pipeline --type classification
```
...
- **wicked-search**: Find model code with `/wicked-search:code "model|classifier"`
- **wicked-kanban**: Track experiments as tasks
- **wicked-data:numbers**: Analyze training data
