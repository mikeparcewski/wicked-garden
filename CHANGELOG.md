...
### Fixed
- **`install.mjs` honours `CLAUDE_CONFIG_DIR` and `--dry-run`; registration is the installer's job (#1117).** `cmdInstall()`/`cmdStatus()` hard-coded `~/.claude/plugins/wicked-garden`, so with `CLAUDE_CONFIG_DIR` pointing at a non-default config dir the copy landed in a dir Claude Code never reads, and `--dry-run` (which wicked-installer passes through) was silently treated as an install. The target is now resolved the way wicked-installer's `install-claude.ts` resolves it: `--claude-home <dir>` (repeatable, also `--claude-home=<dir>`) is the full set → `CLAUDE_CONFIG_DIR` (authoritative; may list several dirs, split on the platform list separator and `,`, `~`-expanded, de-duplicated) → `~/.claude`; the copy lands under `<dir>/plugins/wicked-garden` for EACH resolved dir and `status` reports per dir (config dir + origin, copy version vs package version). `--dry-run` prints what would be copied and whether `uv sync` would run, writes nothing (no `mkdir`, no `uv`) and exits 0; flags are accepted before or after the command, and `pack` verbs keep their verbatim pass-through. After a copy the script says so honestly — `copied (unregistered)` — and points at `npx wicked-installer install wicked-garden`, because Claude Code loads plugins through its marketplace registry and registration is owned by the installer (wicked-installer#18); the script deliberately does NOT grow a second registration code path. Unknown commands, unknown options and stray arguments now exit 2 with usage on stderr instead of printing usage and exiting 0 (`--help`/`-h` print it on stdout); `--dry-run` is an install/update option — `status` is read-only and rejects it instead of silently ignoring it, and `--dry-run`/`--claude-home` given with any other command (`--dry-run pack check`, `--version --dry-run`) are a usage error too, never accepted-and-ignored while the command runs for real (pack verbs own their flags). Hardening from review: `--claude-home` never swallows the next option (`--claude-home --dry-run` is a usage error, not a directory called `--dry-run`); a `CLAUDE_CONFIG_DIR` that is set but names no directory (`:,`, blanks) fails closed with exit 2 instead of silently landing in `~/.claude` (the empty string still counts as unset, the `${CLAUDE_CONFIG_DIR:-~/.claude}` convention); the components garden owns under the config dir (`plugins/`, `plugins/wicked-garden`, `.claude-plugin/`, the manifest) are never followed through a symlink — install and status refuse them (exit 1) and the copied destination's realpath is verified to stay inside the config dir (a symlinked config dir itself stays allowed: dotfiles setups); a failed `uv sync` is reported (`WARNING` + the uv error on stderr), the summary is a non-success and the exit code is 1 — the copy still lands and setup retries, but the run never claims success. npm `bin`/`prepublishOnly` behaviour is unchanged. Tests: `tests/test_install_mjs.py` drives the real CLI against a temp HOME + temp `CLAUDE_CONFIG_DIR` with PATH reduced to an empty dir (`uv` unreachable unless a failing sentinel is planted; hermetic; never touches the operator's config dirs).
