...
### Added
- `skills/engineering/unit-test-quality/` — language- and framework-neutral skill for guarding against useless unit tests. Provides two complementary thought-experiment filters (execution: "replace SUT body with a force-fail (throw/raise/panic) — does the test still pass?" → cuts coverage-only tests; behavior: "replace SUT body with a no-op stub returning defaults — does the test still pass?" → cuts assertion-against-default tests), a taxonomy of seven recurring anti-patterns (tautological / assertion-free / implementation mirror / framework retest / constant verification / sleep-coupled / exception-swallowing) with a one-line decision rule per pattern, a 7-step pre-write self-check, and a reviewer rubric for new tests in PR diffs. Tagged `portability: portable` — applies equally to pytest, Jest, Vitest, JUnit, RSpec, Go's `testing`, and Rust's `#[test]`. Filters are explicitly first-cut diagnostics; per-pattern rules cover the four anti-patterns the filters cannot catch (#1 tautological, #3 implementation mirror, #6 sleep-coupled, #7 exception-swallowing).
...
- `skills/worktrees/` — git worktree hygiene skill covering when to create worktrees, cleanup procedures, and three classes of subagent dangling-commit gotchas (squash-merge cherry false-positive, `git diff` as authoritative content check, ancestry vs patch-id detection). Replaces ad-hoc subagent worktree practices with a discoverable, reviewable contract (#693).
