---
description: "Backward compatibility aliases for old multi-plugin command names"
---

# Command Aliases

If you typed an old command name, here are the new equivalents:

## Crew
| Old | New |
|-----|-----|
| `/wicked-crew:start` | `/wicked-garden:crew:start` |
| `/wicked-crew:execute` | `/wicked-garden:crew:execute` |
| `/wicked-crew:approve` | `/wicked-garden:crew:approve` |
| `/wicked-crew:gate` | `/wicked-garden:crew:gate` |
| `/wicked-crew:status` | `/wicked-garden:crew:status` |
| `/wicked-crew:archive` | `/wicked-garden:crew:archive` |
| `/wicked-crew:evidence` | `/wicked-garden:crew:evidence` |
| `/wicked-crew:just-finish` | `/wicked-garden:crew:just-finish` |
| `/wicked-crew:profile` | `/wicked-garden:crew:profile` |

## Engineering
| Old | New |
|-----|-----|
| `/wicked-engineering:review` | `/wicked-garden:engineering:review` |
| `/wicked-engineering:arch` | `/wicked-garden:engineering:arch` |
| `/wicked-engineering:plan` | `/wicked-garden:engineering:plan` |
| `/wicked-engineering:debug` | `/wicked-garden:engineering:debug` |
| `/wicked-engineering:docs` | `/wicked-garden:engineering:docs` |

## Search
| Old | New |
|-----|-----|
| `/wicked-search:code` | `/wicked-garden:search:code` |
| `/wicked-search:docs` | `/wicked-garden:search:docs` |
| `/wicked-search:blast-radius` | `/wicked-garden:search:blast-radius` |
| `/wicked-search:lineage` | `/wicked-garden:search:lineage` |
| `/wicked-search:index` | `/wicked-garden:search:index` |
| `/wicked-search:refs` | `/wicked-garden:search:refs` |
| `/wicked-search:impact` | `/wicked-garden:search:impact` |
| `/wicked-search:scout` | `/wicked-garden:search:scout` |

## Memory
| Old | New |
|-----|-----|
| `/wicked-mem:store` | `/wicked-garden:mem:store` |
| `/wicked-mem:recall` | `/wicked-garden:mem:recall` |
| `/wicked-mem:forget` | `/wicked-garden:mem:forget` |
| `/wicked-mem:review` | `/wicked-garden:mem:review` |
| `/wicked-mem:stats` | `/wicked-garden:mem:stats` |

## Platform
| Old | New |
|-----|-----|
| `/wicked-platform:security` | `/wicked-garden:platform:security` |
| `/wicked-platform:compliance` | `/wicked-garden:platform:compliance` |
| `/wicked-platform:actions` | `/wicked-garden:platform:actions` |
| `/wicked-platform:health` | `/wicked-garden:platform:health` |
| `/wicked-platform:incident` | `/wicked-garden:platform:incident` |

## Product
| Old | New |
|-----|-----|
| `/wicked-product:elicit` | `/wicked-garden:product:elicit` |
| `/wicked-product:listen` | `/wicked-garden:product:listen` |
| `/wicked-product:analyze` | `/wicked-garden:product:analyze` |
| `/wicked-product:synthesize` | `/wicked-garden:product:synthesize` |
| `/wicked-product:ux-review` | `/wicked-garden:product:ux-review` |
| `/wicked-product:strategy` | `/wicked-garden:product:strategy` |

## QE
| Old | New |
|-----|-----|
| `/wicked-qe:qe` | `/wicked-garden:qe:qe` |
| `/wicked-qe:scenarios` | `/wicked-garden:qe:scenarios` |
| `/wicked-qe:automate` | `/wicked-garden:qe:automate` |
| `/wicked-qe:qe-plan` | `/wicked-garden:qe:qe-plan` |
| `/wicked-qe:acceptance` | `/wicked-garden:qe:acceptance` |

## Data
| Old | New |
|-----|-----|
| `/wicked-data:analyze` | `/wicked-garden:data:analyze` |
| `/wicked-data:ontology` | `/wicked-garden:data:ontology` |

## Delivery
| Old | New |
|-----|-----|
| `/wicked-delivery:report` | `/wicked-garden:delivery:report` |
| `/wicked-delivery:setup` | `/wicked-garden:delivery:setup` |

## Jam
| Old | New |
|-----|-----|
| `/wicked-jam:brainstorm` | `/wicked-garden:jam:brainstorm` |
| `/wicked-jam:jam` | `/wicked-garden:jam:jam` |
| `/wicked-jam:perspectives` | `/wicked-garden:jam:perspectives` |
| `/wicked-jam:council` | `/wicked-garden:jam:council` |

## Kanban
| Old | New |
|-----|-----|
| `/wicked-kanban:board-status` | `/wicked-garden:kanban:board-status` |
| `/wicked-kanban:new-task` | `/wicked-garden:kanban:new-task` |
| `/wicked-kanban:comment` | `/wicked-garden:kanban:comment` |

## Agentic
| Old | New |
|-----|-----|
| `/wicked-agentic:review` | `/wicked-garden:agentic:review` |
| `/wicked-agentic:design` | `/wicked-garden:agentic:design` |
| `/wicked-agentic:audit` | `/wicked-garden:agentic:audit` |
| `/wicked-agentic:frameworks` | `/wicked-garden:agentic:frameworks` |
| `/wicked-agentic:ask` | `/wicked-garden:agentic:ask` |

## Scenarios
| Old | New |
|-----|-----|
| `/wicked-scenarios:run` | `/wicked-garden:scenarios:run` |
| `/wicked-scenarios:list` | `/wicked-garden:scenarios:list` |
| `/wicked-scenarios:check` | `/wicked-garden:scenarios:check` |

## Smaht
| Old | New |
|-----|-----|
| `/wicked-smaht:smaht` | `/wicked-garden:smaht:smaht` |
| `/wicked-smaht:debug` | `/wicked-garden:smaht:debug` |
| `/wicked-smaht:onboard` | `/wicked-garden:smaht:onboard` |

## Startah
| Old | New |
|-----|-----|
| `/wicked-startah:report-issue` | `/wicked-garden:startah:report-issue` |

## Workbench
| Old | New |
|-----|-----|
| `/wicked-workbench:workbench` | `/wicked-garden:workbench:workbench` |
