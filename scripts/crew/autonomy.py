...
def _clarify_via_hitl_judge(
    ...
    try:
        ...
        decision = should_pause_clarify(
            ...
            yolo=True,  # balanced/full both grant yolo to the judge
            env=env,
        )
        return GateDecision(
...
def _check_ac_gate(
    ...
    try:
        ...
        # Determine satisfaction: each AC should have a `satisfied` attribute
        # or a `satisfied_by` list with at least one entry.
        total = len(acs)
        satisfied_count = 0
        ...
        for ac in acs:
            ...
            if hasattr(ac, "satisfied"):
                is_sat = bool(ac.satisfied)
            elif hasattr(ac, "satisfied_by"):
                is_sat = bool(getattr(ac, "satisfied_by", []))
            elif isinstance(ac, dict):
                is_sat = bool(ac.get("satisfied") or ac.get("satisfied_by"))
            else:
                is_sat = False
