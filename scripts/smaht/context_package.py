...
Called by other plugins (wicked-crew, etc.) via direct import or subprocess.
...
# context_package.py is at scripts/smaht/, so parents[1] = scripts/
...
if str(_smaht_dir / "v2") not in sys.path:
    sys.path.insert(0, str(_smaht_dir / "v2"))
...
def discover_installed_plugins() -> list:
    """Discover which wicked-garden domain modules are available.

    Since all plugins are consolidated into scripts/{domain}/, we check
    which domain directories exist under _SCRIPTS_ROOT.
    """
    installed = []
...
async def gather_memories(task: str, limit: int = 3) -> list:
    """Query wicked-brain for task-relevant memories via domain adapter.

    Filters for items emitted by the brain adapter (source == "brain").
    The wicked-garden:mem domain was removed in v8.0.0; brain is the canonical
    memory source.
    """
    try:
