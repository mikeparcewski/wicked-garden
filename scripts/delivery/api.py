...
# delivery/api.py is at scripts/delivery/, so parents[1] = scripts/
...
def _query_crew_projects():
    """Get project completion data from wicked-crew via direct import."""
    try:
...
def _query_crew_signal_stats():
    """Get signal statistics from wicked-crew via subprocess (CLI-only endpoint)."""
    crew_api = _SCRIPTS_ROOT / "crew" / "api.py"
    if not crew_api.exists():
        return None
    try:
...
def _query_mem_stats():
    """Get memory statistics from wicked-mem via direct import."""
    try:
...
def _get_kanban_tasks(project_id=None):
    """Get task data from kanban via direct import."""
    try:
        ...
        if project_id:
            ...
            for p in projects:
                ...
                try:
                    tasks.extend(store.list_tasks(pid))
                except Exception:
                    continue

        return tasks, None
    except Exception as e:
        return None, f"kanban query failed: {e}"
...
def _get_cache():
    """Optionally get wicked-startah cache via direct import."""
    try:
        from startah.cache import namespace
        return namespace("wicked-delivery")
    except Exception:
        return None
...
def _compute_effort_allocation(tasks):
    ...
    for task in tasks:
        ...
        if task_id in task_project_map:
            ...
            is_complete = task.get("swimlane", "") == "done"
            for signal in signals:
                if signal not in by_signal:
                    by_signal[signal] = {"task_count": 0, "completed": 0}
                by_signal[signal]["task_count"] += 1
                if is_complete:
                    by_signal[signal]["completed"] += 1
    ...
    for task in tasks:
        ...
        if any(kw in name_lower for kw in ["feat", "feature", "add", "enhance"]):
            feature_vs_bug["features"] += 1
        elif any(kw in name_lower for kw in ["fix", "bug", "error", "broken"]):
