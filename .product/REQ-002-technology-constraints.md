...
**Hooks** — 13 registered hook events across all `command` type scripts, dispatched through `hooks/scripts/invoke.py`. Primary lifecycle hooks: `SessionStart` (bootstrap: environment setup, peer verification), `UserPromptSubmit` (archetype detection, system-reminder injection), `Stop` (teardown: session audit write). Tool-intercept hooks: `PreToolUse` (tool input validation; MEMORY.md write guard), `PostToolUse`/`PostToolUseFailure` (task sync; file-change tracking). Additional hooks: `TaskCompleted`, `SubagentStart`/`SubagentStop`, `PermissionRequest`, `Notification`, `PreCompact`, `SessionEnd`. Stop and `SessionEnd` hooks use `"async": true`. Bootstrap and Stop hooks return `{"ok": true}` on success; `{"ok": false, "reason": "..."}` blocks the event. The UserPromptSubmit hook returns `{"continue": true}` to proceed (with `hookSpecificOutput` for injected context) or `{"continue": false}` to block. All hook scripts are stdlib-only Python.
